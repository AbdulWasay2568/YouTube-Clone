import { CiSearch } from "react-icons/ci";
import { IoMdMenu } from "react-icons/io";
import { CiBellOn } from "react-icons/ci";
import { FaUserCircle } from "react-icons/fa";
import { RiVideoAddLine } from "react-icons/ri";
import { MdMic } from "react-icons/md";



const Header = () => {
  return (
    <header className="flex justify-between items-center h-14 ml-4 bg-white sticky top-0 w-full">

      {/* Logo Section */}
      <div className="flex items-center">
        <div className="flex items-center h-14">
        <IoMdMenu className="text-xl"/>
          <img
            className="h-14"
            src="data:image/png;base64,"
            alt="YouTube Logo"
          />
          <span className="ml-2 text-xs text-gray-600 font-roboto">PK</span>
        </div>
      </div>

      {/* Search Section */}
      <div className="flex items-center justify-center min-w-[500px] rounded-xl">
        <input
          type="text"
          placeholder="Search"
          name="search"
          className="h-8 w-full max-w-xl px-2 border border-gray-300 text-gray-800 text-sm font-roboto rounded-l-2xl"
        />
        <button
          type="submit"
          className="flex items-center justify-center h-8 w-16 bg-gray-200 border border-gray-300 border-l-0 hover:bg-gray-300 rounded-r-2xl"
        >
          <span
            className="flex text-gray-600 w-6 h-6 items-center justify-center"
            data-icon="ant-design:search-outlined"
            data-inline="false"><CiSearch className="text-xl text-black"/></span>
        </button>
        <button className="ml-4 flex items-center justify-center bg-gray-200 h-8 w-10 text-lg rounded-2xl "><MdMic className=""/></button>
      </div>

      {/* User Section */}
      <div className="flex items-center justify-center w-56 pr-4 bg-white">
        <button className="h-10 w-10 flex items-center justify-center">
          <span className="iconify text-gray-600"><RiVideoAddLine className="text-xl text-black"/></span>
        </button>
        <button className="h-10 w-10 flex items-center justify-center">
          <span className="iconify text-gray-600"><CiBellOn className="text-xl text-black"/></span>
        </button>
        <button className="h-10 w-10 flex items-center justify-center">
          <span className="iconify text-gray-600"><FaUserCircle className="text-2xl"/></span>
        </button>
      </div>
    </header>
  );
};

export default Header;
